...
// example: "EASY_DISPATCH(signal, class_instance, id_of_member)" instead of "dispatch(signal,
// &class_instance, &decltype(class_instance)::id_of_member)"
...
dispatcher<SignalT, SlotArgTs...>::dispatch(const SignalT& signal, void (ClassT::*slot)(SlotArgTs...))
{
  ...
  for (auto& f : map_[signal]) {
    if (reinterpret_cast<size_t>(&slot) == utility::get_address(f))
      exists = true;
  }
  if (!exists)
    map_[signal].push_back(slot);
...
dispatcher<SignalT, SlotArgTs...>::dispatch_bind(const SignalT& signal,
                                                 ...
                                                 ArgTs... bound_args)
{
  map_[signal].push_back([target, slot, bound_args...](SlotArgTs... args) {
    (target->*slot)(bound_args..., std::forward<SlotArgTs>(args)...);
  });
  return map_[signal].size() - 1;
}
...
template<class SignalT, class... SlotArgTs>
bool
dispatcher<SignalT, SlotArgTs...>::poll(size_t limit)
{
  if (event_queue_.size() == 0)
    return false;
  if (limit == 0)
    limit = event_queue_.size();
...
template<class SignalT, class... SlotArgTs>
bool
dispatcher<SignalT, SlotArgTs...>::poll(SignalT filter, size_t limit)
{
  if (event_queue_.size() == 0)
    return false;
  if (limit == 0)
...
template<class SignalT, class... SlotArgTs>
void
dispatcher<SignalT, SlotArgTs...>::calloff(const SignalT& signal)
{
...
template<class SignalT, class... SlotArgTs>
void
dispatcher<SignalT, SlotArgTs...>::calloff(const SignalT& signal, size_t index)
{
...
template<class SignalT, class... SlotArgTs>
void
dispatcher<SignalT, SlotArgTs...>::purge(const SignalT& signal)
{
